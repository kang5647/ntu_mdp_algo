...
class Dubins:
    ...
    def get_obstacles_coordinates(self, obstacles):
        expanded_obstacles = []
        for obs in obstacles:
            x_center = obs['x']
            y_center = obs['y'] 

            expanded_obstacles.append((x_center, y_center))
        return expanded_obstacles

    # Assuming other methods (like dubins_path, lsl, rsr, etc.) are defined here as well...

    def intersects_obstacle(self, path, obstacles):
        """
        Check if the path intersects with any given obstacle.
    
        Parameters
        ----------
        path : numpy.ndarray
            The path as a sequence of points (x, y).
        obstacles : list of tuples
            Each tuple represents an obstacle with format (x, y, radius),
            where (x, y) is the center and radius is the radius of the obstacle.
    
        Returns
        -------
        bool
            True if the path intersects any obstacle, False otherwise.
        """
        for point in path:
            px, py = point
            for obs in obstacles:
                ox, oy, ow, oh = obs  # Obstacle center (x, y) and size (width, height)
                # Calculate the half-width and half-height for easier comparison
                #half_width, half_height = ow / 2, oh / 2
                half_width, half_height = ow , oh 
                # Check if point is within the obstacle bounds
                if (ox - half_width <= px <= ox + half_width) and (oy - half_height <= py <= oy + half_height):
                #if (px - ox) ^ 2 + (py - ox) ^ 2 <= 4:    
                    return True  # Path intersects with this obstacle
                    
        return False  # No intersection with any obstacless
    
    def dubins_path_avoiding_obstacles(self, start, end, obstacles, obstacle_expansion):
        """
        Computes the Dubins path between start and end points avoiding expanded obstacles.
        
        Parameters
        ----------
        start : tuple
            The starting point in the form (x, y, psi), with psi in radians.
        end : tuple
            The ending point in the form (x, y, psi), with psi in radians.
        obstacles : list of tuples
            Obstacle representations as (x, y).
        
        Returns
        -------
        numpy.ndarray
            The shortest list of points (x, y) linking the initial and final points
            avoiding obstacles, or None if no such path exists.
        """
        obstacles_xy = self.get_obstacles_coordinates(obstacles)
        options = self.all_options(start, end)
        feasible_paths = []
        i = 0
        for total_len, dubins_path, straight in options:
            if(total_len == float('inf')):
                continue
            path_points = self.generate_points(start, end, dubins_path, straight)
            collision, obstacle = self.check_collision_with_dynamic_obstacle_expansion(path_points, obstacles_xy, obstacle_expansion)
            if not collision:
                feasible_paths.append((total_len, dubins_path, path_points))
                i += 1  # Move to the next option
    
        if not feasible_paths:
            print(f"No path for {start} and {end}")
            return None  # No feasible path found
    
        # Select the shortest feasible path
        shortest_path_length, dubins_path, shortest_path_points = min(feasible_paths, key=lambda x: x[0])
    
        return shortest_path_length, dubins_path, shortest_path_points
        
    def check_collision_with_dynamic_obstacle_expansion(self, path_points, obstacles, expansion_distance=2):
        """
        Checks if the path collides with any dynamically expanded obstacles.
        
        Parameters
        ----------
        path_points : list of tuples
            The path represented as a list of (x, y) points.
        obstacles : list of tuples
            Obstacles represented as (x, y), assuming circular shape for simplicity.
        expansion_distance : float
            The distance by which the obstacle's radius is expanded for collision check.
        
        Returns
        -------
        tuple
            A tuple (collision_detected, obstacle) if a collision is detected,
            or (False, None) if no collision is found.
        """
        for obstacle in obstacles:
            obstacle_x, obstacle_y = obstacle
            obstacle_unit = 1
            expanded_obstacle_radius = expansion_distance  # Assume radius expansion for simplicity
    
            for path_point in path_points:
                px, py = path_point
                distance_to_obstacle = ((px - obstacle_x)**2 + (py - obstacle_y)**2)**0.5
    
                if distance_to_obstacle <= obstacle_unit + expanded_obstacle_radius:
