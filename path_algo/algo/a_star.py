    ...
    def get_optimal_order_dp(self, retrying) -> List[CellState]:
        ...
        for op in self.get_visit_options(len(all_view_positions)):
            ...
            for c in combination:  # run the algo some times ->
                ...
                # Initialize cost matrix; +1 to include the start node if not already counted
                N = len(visited_candidates)  # Total number of nodes including the start
                cost_np = np.full(
                    (N, N), 1e9
                )  # Initialize with high costs (indicative of no path)
                np.fill_diagonal(cost_np, 0)  # No cost to move from a node to itself

                # Populate the cost matrix
                for s in range(N):
                    for e in range(N):
                        if s != e:  # Skip diagonal
                            u = items[visited_candidates[s]]
                            v = items[visited_candidates[e]]
                            # Fetch the cost between u and v
                            cost_key = (
                                u,
                                v,
                            )  # Assuming (u, v) can uniquely identify the path in self.cost_table
                            cost_np[s][e] = self.cost_table.get(
                                cost_key, 1e9
    ...
    def path_cost_generator(self, states: List[CellState]):
        ...
        def astar_search(start: CellState, end: CellState):
            ...
            while heap:
                ...
                f_distance, cur_x, cur_y, cur_direction = heapq.heappop(heap)
                current_node = (cur_x, cur_y, cur_direction)
                ...
                for next_x, next_y, new_direction, safe_cost in self.get_neighbors(
                    ...
                    next_node = (next_x, next_y, new_direction)
        ...
        # Your existing nested loop for other state pairings
